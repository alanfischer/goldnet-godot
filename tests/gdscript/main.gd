extends Node
## Runner for the GDScript helper suites. Discovers every `suites/suite_*.gd`, runs it, and
## exits non-zero if anything failed. See run.sh.
##
##   godot --headless --path tests/gdscript
##   godot --headless --path tests/gdscript -- --suite interpolation_buffer
...
func _ready() -> void:
	...
	print("")
	if total_failures == 0:
		print("gdscript: %d checks passed across %d suite(s)" % [total_checks, paths.size()])
		get_tree().quit(0)
	else:
		print("gdscript: %d of %d checks failed across %d suite(s)" % [total_failures, total_checks, paths.size()])
		get_tree().quit(1)
...
func _discover(only: String) -> Array[String]:
	...
	for f in dir.get_files():
		...
		if not name.begins_with("suite_") or not name.ends_with(".gd"):
			continue
		if not only.is_empty() and name != "suite_%s.gd" % only:
			continue
		out.append("%s/%s" % [SUITE_DIR, name])
	out.sort()
	return out


func _bail(msg: String) -> void:
	push_error("[gdscript runner] %s" % msg)
	print("FAIL: %s" % msg)
	get_tree().quit(1)
