#!/usr/bin/env bash
set -euo pipefail
# Build the goldnet GDExtension. Cross-compiles via Zig; lipos a universal on macOS.
#
# Usage:
...
#
# Outputs → addons/goldnet/bin/lib<name>.<platform>.template_<cfg>.<arch>.<ext>
# Kept intentionally parallel to hop-godot/build.sh — edit both together.

# ── Per-extension config (the only real difference between the scripts) ───────
ADDON="goldnet"          # addons/<ADDON>/bin
LIB="libgoldnet"         # output library basename
